...
class InstructionResource(Resource):
    ...
    def get(self, instruction_id):
        recipe = next((instruction for instruction in instruction_list
                       if instruction_id == instruction_id and instruction.is_publish == True), None)
        
    def put(self, instruction_id):
        data = request.get_json()
        
        instruction = next((instruction for instruction in instruction_list
                            if instruction.id == instruction_id), None)
